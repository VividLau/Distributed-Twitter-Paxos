...
class Connection:
    ...
    def __init__(self):
        ...
        self.listensock = self.new_socket()
    ...
    def readconfig(self):
        ...
        for node in nodes:
            if node[1] == self_id:
                ...
                self.list_connect_sock.append(self.connectsock_self)
                continue
    ...
    def synod_broadcast(self, broadcast_event, learning=False):
        ...
        while True:
            ...
            if self.prepare_response > 2:
                ...
                for value in self.response_value:
                    if value['accVal'] is None:
                        count_none += 1
                        continue
                    elif value['accNum'] == max_accNum:
                        max_accVal = value['accVal']
                        break
                    else:
                        pass
                if count_none == len(self.response_value):
                    max_accVal = None
    ...
    def learn(self):
        ...
        if self.is_updated is True:
            ...
            print('fail to update the whole log!')
            print()
        self.logid = []

    def new_socket(self):
        s_ock = socket(AF_INET, SOCK_STREAM)

        # release the port immediately after close the socket
        s_ock.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        return s_ock
