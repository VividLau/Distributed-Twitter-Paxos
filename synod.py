...
class Connection:
    ...
    def synod_accept(self, client_sock, decode_data):
        ...
        # accept msg as an acceptor:
            ...
            print('receive prepare number %d, max prepare num is %d' % (decode_data['prepare'], self.Max_prepare))
            print()
            ...
            print('receive accNum number %d, max prepare num is %d' % (decode_data['accNum'], self.Max_prepare))
            print()
            ...
            print('receive commit(v) from proposer')
            print()
    ...
    def synod_broadcast(self, broadcast_event):
        ...
        while True:
            ...
            if self.leader is True:
                ...
                print('send accept(0, v) to all acceptor as a leader')
                self.broadcast(leader_acc)
                time.sleep(2)
                print('receive %d response(s) to accept(0, v)' % self.ack_response)
                if self.ack_response > 2:
                    commit = {'commit': 'commit', 'value': leader_acc['accVal']}
                    print('sent commit(v) to all acceptors')
            ...
            if self.prepare_response < 3:
                ...
                print('send prepareNum: %d to all acceptors' % prepareNum)
                self.broadcast(prepare)
                time.sleep(1.5)
                print('received %d response(s) to prepare' % self.prepare_response)
                print()
                count += 1
                m += 1
                if count > 5:
            ...
            if self.prepare_response > 2:
                ...
                print('sent accept(n, v) to all acceptors')
                print()
                self.broadcast(accept)
                time.sleep(2)
                print('received %d response(s) to accept(n, v)' % self.ack_response)
                print()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice == 'tweet':
            ...
            t_event = {'name': cinstance.id_self, 'tweet': tweet, 'time': t_time}
        ...
        if choice == 'block':
            ...
            b_event = {'name': cinstance.id_self, 'block': block_id, 'time': b_time}
        ...
        if choice == 'unblock':
            ...
            u_event = {'name': cinstance.id_self, 'unblock': unblock_id, 'time': u_time}
