...
class Connection:
    ...
    def synod_broadcast(self, event):
        # Synod algorithm for proposers
        m = 0
        count = 0
        cinstance.prepare_response = 0
        cinstance.ack_response = 0
        while True:
            ...
            if self.prepare_response > 2:
                ...
                if max_accVal is None:
                    accept = {'accept': 'accept', 'accNum': prepareNum, 'accVal': event}
                else:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice == 'tweet':
            ...
            tweet = input('')
            t_event = {'name': cinstance.id_self, 'tweet': tweet, 'time': datetime.datetime.now()}
            cinstance.synod_broadcast(t_event)

        if choice == 'block':
            print('Who do you want to block?')
            block_id = input('')
            b_event = {'name': cinstance.id_self, 'block': block_id, 'time': datetime.datetime.now()}
            cinstance.synod_broadcast(b_event)

        if choice == 'unblock':
            print('Who do you want to unblock?')
            unblock_id = input('')
            u_event = {'name': cinstance.id_self, 'unblock': unblock_id, 'time': datetime.datetime.now()}
